 ...
 * @api
 * @version 0.4
 * @since 2.0 First time this was introduced into MailChimp Form plugin.
...
class cUsComAPI_MC {
     ...
     * @api
     */
    var $enviroment = cUsMC_API_ENV;
    var $wpApiAccountKey = cUsMC_API_ACC;
    var $wpApiKey = cUsMC_API_AKY;
    ...
    public function getAPICredentials($cUs_email, $cUs_pass) {
        ...
        if( $this->_isCurl() ){
            ...
            curl_setopt($ch, CURLOPT_HTTPHEADER, array('X-ContactUs-Request-URL: ' . $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI'], 'X-ContactUs-Signature: wpmc|3.0|' . $this->getIP()));
        ...
        if (!strlen($postData['fname'])) {
            ...
            if( $this->_isCurl() ){
                ...
                curl_setopt($ch, CURLOPT_HTTPHEADER, array(
                    ...
                    'X-ContactUs-Signature: wpmc|3.0|' . $this->getIP()
                ));
    ...
    public function getDefaultDeepLink($cUsMC_API_getFormKeys) {

        if (empty($cUsMC_API_getFormKeys))
            return FALSE;

        //$cUsMC_API_getKeysResult = $this->getFormKeysData($cUs_API_Account, $cUs_API_Key);
        $cUs_jsonKeys = json_decode($cUsMC_API_getFormKeys);
    ...
    public function resetData() {
        ...
        delete_option('cUsMC_settings_FORM');
        ...
        delete_option('cUsMC_settings_default_deep_link_view');
        delete_option('cUsMC_settings_form_id');
        delete_option('cUsMC_settings_intro_hints');

        cUsMC_page_settings_cleaner(); //REMOVE CUSTOM SETTINGS BY PAGE
        cUsMC_inline_shortcode_cleaner(); //REMOVE SHORTCODES
