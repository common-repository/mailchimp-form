...
cUsMC_myjq(document).ready(function($) {
    ...
    $('.introjs-skipbutton').on('click',function(){
        ...
        bootbox.confirm("User guide will appear again using the help button above.", function(result) {
            if(result){
                $.ajax({ type: "POST", url: ajax_object.ajax_url, data: {action:'cUsMC_disable_introjs'},
    ...
    intro.setOptions({
        steps: [
            ...
            },
            {
                ...
                intro: "<div class='intro_box_ai intro'><h1>Automatic Synching with your MailChimp account</h1><ul><li>The form captures the lead and sends it to specific MailChimp lists automatically.</li><li>You can set it to no op-in, opt-in, double opt-in and also add CAPCHA to avoid spam.</li></ul></div>",
                position: "bottom"
            },
            {
                //8
